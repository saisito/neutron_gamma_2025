...
data_dir = "preprocessed_data_salt"
...
def train_and_tune_model(model, param_grid, X_train, y_train, model_name, n_iter=50, cv=5):
    ...
    try:
        ...
        model_path = f"results_salt/models/{model_name}_tuned_salt.pkl"
...
joblib.dump(voting_clf, 'results_salt/models/voting_salt.pkl')
...
plt.grid(alpha=0.3)
plt.savefig('results_salt/thresholds_salt.png', dpi=300)
...
output_csv_path = "results_salt/models/voting_salt_metrics.csv"
...
plt.savefig('results_salt/confusion_salt.png', dpi=300)
...
plt.legend(loc="lower right", fontsize=16)
plt.tick_params(axis='both', labelsize=20)
plt.savefig('results_salt/roc_salt.png', dpi=300)

scaler = joblib.load('results_salt/models/scaler_salt.pkl')

# Assuming the pipeline has been trained and saved, and you have a pre-fitted scaler
best_model = voting_clf

# Define data file
desnuda_file = 'filtered_AmBe_salt/AmBe_desnuda.csv'
...
os.makedirs("predicted_salt", exist_ok=True)
save_pulses_to_csv(neutron_pulses_desnuda, gamma_pulses_desnuda, 'predicted_salt/neutron_pulses_salt.csv', 'predicted_salt/gamma_pulses_salt.csv')
